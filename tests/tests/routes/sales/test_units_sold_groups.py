"""Тесты для GET /sales/units_sold_groups."""
...
async def test_es_connection_error_returns_503(
    ...
    elastic_service_mock.sales.raise_on_units_sold_groups = ConnectionError("cluster down")

    response = await test_client_no_db.get("/sales/units_sold_groups")
...
async def test_passes_filters_to_service(
    ...
    last_call = elastic_service_mock.sales.units_sold_groups_calls[-1]
...
async def test_no_params_returns_all(
    ...
    await test_client_no_db.get("/sales/units_sold_groups")
