...
async def test_search_missing_query_param_returns_422(test_client):
    ...
    response = await test_client.get("/documents/search")
...
async def test_search_db_operational_error_returns_503(test_client, elastic_service_mock):
    """Ошибка соединения с БД — 503."""
    elastic_service_mock.documents.set_search_result("test", [1])
    with patch(
        "src.db.repository.document.DocumentRepository.get_by_ids",
        side_effect=OperationalError("connection refused", None, None),
    ):
        response = await test_client.get("/documents/search", params={"q": "test"})
        assert response.status_code == 503
        assert response.json()["detail"] == "Сервис недоступен"


async def test_search_es_error_returns_503(test_client, elastic_service_mock):
    """Ошибка Elasticsearch — 503."""
    elastic_service_mock.documents.raise_on_search = ConnectionError("cluster down")
    response = await test_client.get("/documents/search", params={"q": "test"})
    assert response.status_code == 503
    assert response.json()["detail"] == "Сервис недоступен"
...
async def test_search_returns_404_when_es_returns_nothing(test_client):
    ...
    response = await test_client.get("/documents/search", params={"q": "nothing"})
    assert response.status_code == 404
    assert "Документы по заданному запросу не найдены" in response.json()["detail"]


async def test_search_returns_404_when_docs_not_in_db(test_client, elastic_service_mock):
    """ES вернул id, но документов с такими id нет в БД — 404."""
    elastic_service_mock.documents.set_search_result("missing", [99999])
    response = await test_client.get("/documents/search", params={"q": "missing"})
    assert response.status_code == 404
...
async def test_search_results_sorted_by_created_date_desc(
    test_client, db_operations, data_generator, elastic_service_mock,
):
    ...
    elastic_service_mock.documents.set_search_result("test", ids)
...
async def test_search_returns_matching_documents(
    ...
    elastic_service_mock.documents.set_search_result("hello", [doc1.id])
    ...
    assert elastic_service_mock.documents.search_calls == [{"query": "hello"}]
